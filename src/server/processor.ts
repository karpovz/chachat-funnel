...
export function validateCard(card: PurchaseInput["card"], now = new Date()) {
  ...
  let sum = 0;
  for (
    let i = number.length - 1, alternate = false;
    i >= 0;
  ...
  if (!/^\d{12,19}$/.test(number) || sum % 10 !== 0 || /^0+$/.test(number))
    throw new ApiError(400, "invalid_card", "Enter a valid card number.");
  const expiry = /^(\d{2})\s*\/\s*(\d{2}|\d{4})$/.exec(card.expiry);
  const month = Number(expiry?.[1]);
  const year = Number(expiry?.[2]) + (expiry?.[2]?.length === 2 ? 2000 : 0);
  if (
    !expiry ||
    month < 1 ||
    month > 12 ||
  ...
  if (!/^\d{3,4}$/.test(card.cvc))
    throw new ApiError(400, "invalid_cvc", "Enter a valid security code.");
  return {
  ...
  try {
    return await Promise.race([
      new Promise<SafeCard["outcome"]>((resolve) => {
        ...
        if (card.outcome !== "timed_out")
          operation = setTimeout(() => resolve(card.outcome), 450);
      }),
      new Promise<SafeCard["outcome"]>((resolve) => {
        deadline = setTimeout(() => resolve("timed_out"), 1500);
      }),
    ]);
  } finally {
